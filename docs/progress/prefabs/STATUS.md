...
5. **Catalogue fil additif** — `PublishedCatalogResult` JSON : champs optionnels `prefabs` (`id`, variantes, `pngBase64`) et `prefabMaps` (`mapId`, `mapName`, `runtimeMapId` optionnel, placements). Pas de bump `FrogWireProtocol`. Un PNG réel dépasse l’ancien préfixe `UInt16` (64 KiB) : le serveur fragmente alors (`0xFFFF`) sans changer `FrogWireProtocol.Version` (reste 11). `runtimeMapId` vient de `runtime_map_bindings` et n’est pas effacé si le cache monde est froid.
