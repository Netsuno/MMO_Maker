...
/// <summary>
/// Shell « Données de jeu » : navigation catégories + éditeur tilesets (Phase 6 slice 1).
...
public sealed class GameDataForm : Form
{
    ...
    public GameDataForm()
    {
        ...
        _tilesets = new TilesetEditorPanel(new TilesetWorkspaceSession(bundle.Repository), bundle.Capabilities);
        _tilesets.StatusChanged += msg => _status.Text = msg;

        _categoryList.Items.AddRange(new object[]
        {
            "Tilesets",
            "NPCs / monstres (Phase 6 — à venir)",
        ...
        Load += async (_, _) => await _tilesets.InitializeAsync().ConfigureAwait(true);
    }
