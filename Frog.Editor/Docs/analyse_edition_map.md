...
- Colonne **`trigger_kind`** sur `frog_map_event` : **`interact`** (action « Interagir » / paquet 31), **`step_on`** (à l’arrivée sur la tuile après mouvement/sync), **`page`** (une fois par entrée sur la carte, tuile d’arrivée) ou **`auto_tile`** (sur place : `HeartbeatRequest` serveur, cooldown par placement). Combo **Déclencheur** + **Appliquer déclencheur à la ligne** dans le dialogue. Sur le canevas / mini-carte : pastille haut-droite ronde = `interact`, losange = `step_on`, petit cercle bas-gauche = `page`, rectangle pointillé tuile = `auto_tile` (couleurs inchangées par slug).
