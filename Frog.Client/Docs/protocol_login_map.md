...
Champs optionnels **`prefabs`** / **`prefabMaps`** : définitions + `pngBase64` et placements par carte, depuis le jsonb `prefabs_json` des snapshots cartes publiés (PostgreSQL) ou le sidecar playtest `published-prefabs.json`. Le client écrit `Prefabs/{sprite}.png`, `Prefabs/catalog.json` et `Maps/{nom}.prefabs.json` (exe + cwd) puis `ClientPrefabLoader` / `DrawPlacedPrefabs`.
