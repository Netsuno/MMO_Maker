...
/// <summary>Session éditeur Système : catalogue interrupteurs / variables + brouillon courant.</summary>
public sealed class SystemFlagWorkspaceSession
{
    ...
    /// <summary>Liste et nouveaux brouillons limités à ce type (interrupteur ou variable).</summary>
    public SystemFlagKind KindFilter { get; set; } = SystemFlagKind.Switch;
    ...
    /// <summary>Suffixe <c>_copie</c> compatible avec les clés de commande.</summary>
    internal static string CopyKey(string key)
    {
        const string suffix = "_copie";
        var stem = key?.Trim() ?? string.Empty;
        var room = CharacterPayloadWorldFlags.MaxKeyUtf8Bytes - Encoding.UTF8.GetByteCount(suffix);
    ...
    public static SystemFlagDefinition Clone(SystemFlagDefinition source) => new()
    {
        Id = source.Id,
        Kind = source.Kind,
        Key = source.Key,
