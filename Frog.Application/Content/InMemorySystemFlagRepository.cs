...
public sealed class InMemorySystemFlagRepository : ISystemFlagRepository, IPublishedSystemFlagCatalog
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (request.FlagId is not Guid existing || existing == Guid.Empty)
        {
            ...
            {
                return Task.FromResult<SaveSystemFlagResult>(
                    new SaveSystemFlagResult.ValidationFailed(DuplicateKeyMessage(definition.Kind)));
            }
        ...
        return Task.FromResult<SaveSystemFlagResult>(
            new SaveSystemFlagResult.Success(newRevision, id, publishedRevision));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (kind is { } filter)
        {
            query = query.Where(p => p.Definition.Kind == filter);
        }
    ...
    {
        foreach (var draft in _drafts.Values)
        {
            if (exceptId is Guid id && draft.Id == id)
            {
                continue;
            }

            if (draft.Definition.Kind == kind
        ...
        foreach (var published in _published.Values)
        {
            if (exceptId is Guid id && published.Id == id)
            {
                continue;
            }

            if (published.Definition.Kind == kind
                && string.Equals(published.Definition.Key, key, StringComparison.Ordinal))
            {
                return true;
            }
        }
    ...
    private sealed record PublishedRecord(Guid Id, SystemFlagDefinition Definition, long Revision);
}
