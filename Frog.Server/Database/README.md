...
Tables : `accounts`, `player_world_state` (héritage), **`character_world_state`** (position par perso), `frog_map`, `frog_character`, `frog_asset_blob`, `frog_map_editor_save`, **`frog_event_catalog`**, **`frog_map_event`**, **`frog_item_definition`**, **`character_inventory_slot`**, **`character_stat`**, **`character_world_flag`**, **`character_payload_kv`** (`entry_value` **LONGTEXT** UTF-8 — pas de type JSON SQL sur le perso après **v10**).
...
3. Lance le serveur une fois : **`MariaDbSchemaBootstrap`** exécute ce script au démarrage, puis FK `fk_pws_character`, puis **migration v2**, puis migrations **v3–v10** (dont inventaire v7, stats/flags v8, extras KV v9, suppression JSON perso v10), puis seed `demo` / carte selon ta config.
