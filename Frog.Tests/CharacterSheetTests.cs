...
public sealed class CharacterSheetTests
{
    ...
    public void Shell_TogglesSheetFromKeyAndPerso_WithoutProtocolBump()
    {
        ...
        Assert.Contains("CharacterSheetGear.IsEquippable", sheet, StringComparison.Ordinal);
        Assert.Contains("CharacterSheetGear.FormatBagRow", sheet, StringComparison.Ordinal);
        Assert.Contains("event Action<byte>? EquipRequested", sheet, StringComparison.Ordinal);
        var inventory = File.ReadAllText(Path.Combine(RepoRoot(), "Frog.Client", "Controls", "InventoryPanel.cs"));
        var equipment = File.ReadAllText(Path.Combine(RepoRoot(), "Frog.Client", "Controls", "EquipmentPanel.cs"));
        Assert.Contains("CharacterSheetGear.IsEquippable", inventory, StringComparison.Ordinal);
        Assert.Contains("CharacterSheetGear.FormatBagRow", inventory, StringComparison.Ordinal);
        Assert.Contains("ItemTypeLookup", inventory, StringComparison.Ordinal);
        Assert.Contains("_inventoryPanel.ItemTypeLookup = ResolveItemType", shell, StringComparison.Ordinal);
        Assert.Contains("_inventoryPanel.RefreshPresented()", shell, StringComparison.Ordinal);
        Assert.Contains("_equipmentPanel.RefreshPresented()", shell, StringComparison.Ordinal);
        Assert.Contains("RefreshPresented()", equipment, StringComparison.Ordinal);
        Assert.Contains("Arme:", equipment, StringComparison.Ordinal);
        Assert.Contains("Armure:", equipment, StringComparison.Ordinal);
