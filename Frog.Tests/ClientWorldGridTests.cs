...
public sealed class ClientWorldGridTests
{
    ...
    public void MapViewRenderer_TileGridDefaultsOff_PlayPathDoesNotEnable()
    {
        ...
        Assert.Contains("MapViewRenderer.Render(", shellText, StringComparison.Ordinal);
        Assert.Contains("localPose: localPose", shellText, StringComparison.Ordinal);
        Assert.Contains("otherPoses: otherPoses", shellText, StringComparison.Ordinal);
        Assert.Contains("prefabPlacements: _prefabPlacements", shellText, StringComparison.Ordinal);
    }
