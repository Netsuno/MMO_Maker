...
public sealed class Phase7InMemorySmokeE2ETests
{
    ...
    public async Task FullGameplayFlow_RegisterLoginCreateSelectFightShopReconnect()
    {
        ...
        try
        {
            ...
            await clientShop.SendFrameAsync(BuildShopBuy(Phase7ContentSeed.DefaultShopId, Phase7ContentSeed.DefaultItemId, 1, Guid.NewGuid()));
            Assert.NotEqual(0, (await clientShop.ReadUntilAsync(PacketId.ShopBuyResult))[1]);

            await clientShop.SendFrameAsync(BuildBankDepositItem(0, 1, Guid.NewGuid()));
            Assert.NotEqual(0, (await clientShop.ReadUntilAsync(PacketId.BankDepositResult))[1]);

            await clientShop.SendFrameAsync(BuildBankWithdrawItem(0, 1, Guid.NewGuid()));
    ...
    [Fact]
    [Trait("Category", "InMemorySmoke")]
    public async Task WorldFlagsPatchRequest_RejectedInProductionComposition()
    {
        var port = GetFreePort();
        using var host = FrogServerHostFactory
            .CreateHostBuilder(
        ...
        try
        {
            var user = $"p9wf-{Guid.NewGuid():N}"[..20];
            const string password = "password123";
            await using var client = new Phase7TcpClient();
            await client.ConnectAsync("127.0.0.1", port);
            Assert.Equal((byte)PacketId.Hello, (await client.ReadFrameAsync())[0]);
            ...
            await client.SendFrameAsync(BuildCharacterCreate("FlagsForge", Phase7ContentSeed.DefaultClassId));
            var create = await client.ReadUntilAsync(PacketId.CharacterCreateResult);
            Assert.NotEqual(0, create[1]);
            var characterId = Encoding.UTF8.GetString(create, 3, create[2]);
            ...
            await client.SendFrameAsync(BuildWorldFlagsPatch("""{"story_intro":true}"""));
    ...
    private static byte[] BuildWorldFlagsPatch(string patchJsonObject)
    {
    ...
    private static byte[] BuildShopBuy(Guid shopId, Guid itemId, int qty, Guid requestId)
    {
    ...
    private static byte[] BuildBankDepositItem(byte slot, int qty, Guid requestId)
    {
    ...
    private static byte[] BuildBankWithdrawItem(byte slot, int qty, Guid requestId)
    {
        var payload = new byte[1 + 1 + 4 + 16];
